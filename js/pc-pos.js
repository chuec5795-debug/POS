...
function givingReceipt() {
  ...
  let jsonString = JSON.stringify(receiptObject);
  let encodedReceipt = btoa(encodeURIComponent(jsonString));
 // 1. Detect if a custom override exists in window.LOCAL_TUNNEL_URL (from config.js)
  // 2. Otherwise default to window.location.origin (automatically captures active host/tunnel)
